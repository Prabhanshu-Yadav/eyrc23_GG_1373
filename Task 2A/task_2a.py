...
# Team ID:			1373
# Author List:		Ayush Tiwari, Bhagyesh Dhamandekar, Om Rajput, Prabhanshu Yadav
# Filename:			task_2a.py
...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    marker_corners,marker_ids,_ = aruco.detectMarkers(gray_frame,aruco_dict,parameters = param)
    marker_ids1=marker_ids.flatten()
    for i in range(len(marker_ids)):
        reshaped_corners  = marker_corners[i].reshape((4, 2))
        ArUco_corners.update({int(marker_ids[i][0]): reshaped_corners})

    for keys in ArUco_corners:
        corners= ArUco_corners[keys]
        id = ArUco_corners[keys]
        AB_vector = (id[1][0] - id[0][0], id[1][1] - id[0][1])
        
        angle_of_orientation = math.degrees(math.atan2(AB_vector[1], AB_vector[0]))
        if (angle_of_orientation<0 and angle_of_orientation>=-180):
            angle_of_orientation= angle_of_orientation*-1
        elif(angle_of_orientation>0 and angle_of_orientation<=180):
            angle_of_orientation=angle_of_orientation*-1
        else:
            angle_of_orientation=angle_of_orientation
    
        centres = [[int(0.25*(corners[0][0]+corners[1][0]+corners[2][0]+corners[3][0])),int(0.25*(corners[0][1]+corners[1][1]+corners[2][1]+corners[3][1]))],int(angle_of_orientation)]

        ArUco_details_dict.update({keys:centres})
